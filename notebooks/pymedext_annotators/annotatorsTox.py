...
class StanzaAnnotator(Annotator):
    ...
    def __init__(self, key_input, key_output, ID,
        ...
        self.nlp = stanza.Pipeline(lang = lang, processors = processors) # on crée un pipeline stanza avec les différents process en paramètres 
        # Pour mieux comprendre, annoter des phrases avec le tuto : https://stanfordnlp.github.io/stanza/depparse.html
    ...
    def annotate_function(self, _input):
        ...
        for el in inp:
            ...
            # Premier parcours ; on crée une annotation pymedext par mot
            for sent in stanza_doc.sentences :
                dict_word_ID_to_ann_ID = {}
                list_id_par_rel=[]
                # on crée un annotation pymedext par mot dans les phrase
                for word in sent.words :
                    attributes = word.to_dict() # toutes les annotations Stanza pour le mot en question seront en attributs
                    # Ici on règle le problème de de multi-world-expansion (MWT) cf https://stanfordnlp.github.io/stanza/mwt.html
                    if 'misc' in attributes : # Cas 1: le mot n'est pas décomposable en plusieurs mots syntaxiques, on n'aura donc pas de problème de span
                        span0,span1 = int(el.span[0]) + int(attributes['misc'].split('|')[0].split("=")[-1]), int(el.span[0]) + int(attributes['misc'].split('|')[1].split("=")[-1]) # le span se trouve dans misc
                        ann = Annotation(
                            type = self.key_output,
                            value = word.text,
                            span = (span0, span1),
                        ...
                        res.append(ann)
                    else : # Cas 2 : on est sur un mot du type "du" = "de"+"le", il faut régler les problèmes de span
                        if word.parent.id not in dict_word_ID_to_ann_ID :
                            ...
                            span0,span1 = int(el.span[0]) + int(attributes_parent['misc'].split('|')[0].split("=")[-1]), int(el.span[0]) + int(attributes_parent['misc'].split('|')[1].split("=")[-1]) # on prend le span du mot parent
                            ann = Annotation(
                                type = self.key_output,
                ...
                # Premier parcours ; on crée des relations entre ces annotations avec le parser de dépendance
                for word in sent.words : 
